...
class set {
	...
	public:
		...
		typedef typename ft::node_iterator<const value_type, node_type>			iterator;
	...
	public:
		...
		count (const key_type& k) const {
			return (!(_t.find(k) == _nullptr));
		}
		
		iterator
		find (const key_type& k) {
			return (iterator(_t.find(k), ROOT_, END_));
		}
		
		const_iterator
		find (const key_type& k) const {
			return (const_iterator(_t.find(k), ROOT_, END_));
		}

		iterator
		lower_bound (const key_type& k) {
			return (iterator(_t.lower_bound(k), ROOT_, END_));
		}

		const_iterator
		lower_bound (const key_type& k) const {
			return (const_iterator(_t.lower_bound(k), ROOT_, END_));
		}

		iterator
		upper_bound (const key_type& k) {
			return (iterator(_t.upper_bound(k), ROOT_, END_));
		}
